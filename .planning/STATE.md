...
- [Phase 03.1-06]: Usuario aprobó "desplegar-ahora" sabiendo, porque se le dijo antes de decidir, que el despliegue llevaba dentro el cierre de siete brechas de DC-3 (`aaaf8d5`), commiteado por debajo de los seis planes de esta fase por la sesión `prol-1d`. Producción corre desde el 2026-09-03 la imagen `9bf55ee`, con las cinco sentencias de esquema (dos de esta fase, tres de DC-3) aplicadas antes de mover `latest`.
...
- **La instancia de PROL sigue sin levantar tras la migración v1.2 (2026-09-04).** `https://prol.prosuite.pro/api/health` devuelve **502**: no existe la unit `prol-web.service`, no existe el tag `localhost/prol-web:prol`, y la unit anterior `prol-web-1` se detuvo deliberadamente (SIGTERM, exit 143) el 2026-09-04 a las 05:29. Su base `prol-db-1` sigue corriendo pero el rol `prol` ya no existe en el volumen `prol_prol_db_data` — el único rol es `ibiza`, así que su env (`POSTGRES_USER=prol`) y el `HealthCmd` del quadlet (`pg_isready -U prol -d prol`) tampoco cuadran; es exactamente la trampa anotada en `9ca296c`. **Confirmado por el usuario como paso pendiente, no como caída imprevista.** Ibiza sí está sana: `ibiza-web` `healthy` sobre `prol-web:55df7c2`, dominio en 200 y redirect legacy funcionando. Fuera del alcance de la fase 4, que despliega sólo a `ibiza`.
- **La base de la instancia dedicada de Ibiza contiene los tres tenants (2026-09-04).** `ibiza-db` (volumen propio `ibiza_db_data`, 74M) tiene `academia-digital`, `ibiza-online` y `mecanica-g3`, no sólo el suyo. La aplicación sirve únicamente `ibiza-online` por `DEFAULT_TENANT_SLUG`, así que no hay fuga visible, pero los datos de dos clientes ajenos viven en la instalación dedicada de un tercero — en un producto de cumplimiento eso merece una decisión explícita. **El usuario pidió anotarlo y no tocar datos por ahora**; queda pendiente decidir si se purgan o si la copia íntegra es deliberada.
...
- **Confirmación visual humana sobre producción, pendiente (03-08, y de nuevo en 03.1-06).** Falta que el usuario entre a `https://prol.prosuite.pro`, vea el panel normal y descargue algo que ya funcionara antes (certificado o PDF de resultados). Sigue sin ocurrir tras dos despliegues consecutivos. No bloqueante: todo lo automatizable ya se confirmó dos veces y el rollback de un comando (`podman tag localhost/prol-web:04135ca localhost/prol-web:latest && systemctl restart prol-web-1.service`) está listo si algo apareciera mal.
- **Confirmación visual humana sobre el menú agrupado de IBIZA en producción, pendiente (03.1-06).** El checkpoint final del despliegue de la fase 3.1 se aprobó sin contestar ninguna de las tres preguntas del `resume-signal`. Falta que un administrador de IBIZA entre a producción y confirme que el sidebar muestra un desplegable (no cuatro entradas sueltas) con sus cuatro hijos y el rótulo esperado. No bloqueante: lo automatizable está confirmado (columna nullable, 0 tenants con rótulo propio, servicio sano). Ver `DEPLOY.md` §7e y `03.1-06-SUMMARY.md`.
...
- **DRV-02 pendiente (03.1-04, reconfirmado en 03.1-06).** El checkpoint que debía confirmar que el clic hacia "Abrir la carpeta de Drive" aterriza en una carpeta real se aprobó dos veces sin ejercitarse en 03.1-04: falta que un usuario con una cuenta de Drive real entre como administrador, pegue su enlace en un proyecto real, y confirme que el botón lo lleva ahí desde el panel del cliente y desde la ficha de revisión del consultor. Toda la evidencia server-side (href idéntico a la columna, sin fuga entre empresas) ya está cerrada. **El despliegue a producción de 03.1-06 no puede cerrarlo tampoco**: `manual_assignments` está en 0 filas en producción, así que no existe ningún proyecto donde haya un enlace de Drive real que abrir. Ver `03.1-04-SUMMARY.md` §"Tarea 3: aprobado sin ejercitar" y `03.1-06-SUMMARY.md`.
...
## Estado de producción (actualizado 2026-09-03, plan 03.1-06)

- **Desplegado `9bf55ee` en `panel-prosuite-2`** (venía de `04135ca`; anteriores siguen tagueadas para rollback de más niveles: `55c020d` y `64f7476`). Los alias `panel-prosuite-2` y `propodvps2` resuelven al **mismo host** (`195.26.255.71`, hostname real `propodvps2`). Rollback de imagen: `podman tag localhost/prol-web:04135ca localhost/prol-web:latest && systemctl restart prol-web-1.service` (no ejecutado, no hizo falta).
- **El usuario aprobó "desplegar-ahora" sabiendo que el despliegue lleva dentro el cierre de siete brechas de DC-3** (`aaaf8d5 feat(dc3): cerrar las siete brechas del modulo DC-3`), commiteado por la sesión `prol-1d` por debajo de los seis planes de esta fase — fijar el SHA de esta fase no lo excluye.
- **Esquema de la fase 3.1 y de DC-3 APLICADO en producción.** `db push` con el schema que trae la imagen `9bf55ee`, ejecutado **antes** de mover `latest`: cinco sentencias, todas aditivas — `tenants.documents_menu_label` y `manual_assignments.drive_url` (fase 3.1, ambas `TEXT` nullable sin default), y `dc3_course_editions.company_id` con su índice y su FK a `companies` (DC-3). Ni `DROP`, ni `SET NOT NULL`, ni enum nuevo, ni `--accept-data-loss` pedido en ningún momento. Dump previo en `/opt/prol/backup_20260903_0404_pre_fase31.sql` (3.4M).
- **`documents_enabled` no se tocó.** Idéntico antes y después en los tres tenants: `ibiza-online=true`, `academia-digital=false`, `mecanica-g3=false`. Este despliegue no encendió ni apagó nada.
- **Verificación automatizada completa**, confirmada por este agente y re-confirmada de forma independiente por el orquestador: servicio activo, `/api/health` y `/sign-in` en 200, las dos columnas nullable sin default, 0 tenants con `documents_menu_label` no nulo, `EvidenceRequirementKind` sin cambios (3 valores), `POST /api/upload/evidence` en 404 (retirada en 03.1-05), imágenes `9bf55ee`/`04135ca`/`55c020d`/`64f7476` tagueadas, log de arranque limpio a las `06:10:46`, ninguna credencial filtrada. `manual_assignments` y `evidences` en 0 filas cada una en producción.
- **Backend de almacenamiento confidencial: R2 sigue activo**, sin cambios en este despliegue.
- **Confirmación visual humana sobre producción: PENDIENTE, otra vez.** El checkpoint final se aprobó con un "aprobado" sin contestar ninguna de las tres preguntas del `resume-signal` (¿se descargó un certificado o PDF?, ¿un desplegable o cuatro entradas sueltas en el sidebar de IBIZA?, ¿qué rótulo exacto?), pese a habérsele dicho explícitamente antes de responder que así se registraría como "aprobado sin ejercitar". Se registra exactamente así: **nadie ha visto el sidebar de IBIZA en producción**, y **la deuda heredada del plan 03-08** (descargar un certificado o PDF) **sigue sin saldarse** — van ya dos despliegues seguidos sin ese paso. Es la tercera vez consecutiva en esta fase que un checkpoint de verificación humana se aprueba sin ejercitarse (03.1-04 y 03.1-06; sólo 03.1-02 se ejerció de verdad). Ver `DEPLOY.md` §7e y `03.1-06-SUMMARY.md`.
