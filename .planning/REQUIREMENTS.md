...
| DOC-03 | Phase 3 | Complete (03-05: `issueCompanyDocument` congela `contentHtml` y `sourceTemplateVersion` al emitir, dentro de la misma transacción con lock que degrada la `VIGENTE` anterior. Verificado contra la base real: editar la plantilla después de emitir a dos empresas no movió ni un carácter de los dos snapshots, comparados antes/después. **Advertencia de precisión**: la verificación fue por script desechable que replica la forma de la transacción — las server actions son `"use server"` y no se invocan por `curl` — no por invocar `issueCompanyDocument` de verdad; eso lo hace el plan 03-06 por la interfaz) |
...
| DOC-05 | Phase 3 | Complete (03-05: `getCompanyDocumentForClient`/`getCompanyDocumentForEdit` generan el historial —versión, fecha, autor, `notes` como descripción del cambio, estatus— en tiempo de render desde las filas de `CompanyDocument`, sin que nadie lo redacte a mano. Verificado contra la base real: cuatro filas de historial para Acme tras el ciclo completo, con las cinco columnas rellenas y al menos una descripción de cambio real) |
| DOC-06 | Phase 3 | Complete (03-05: `startCompanyDocumentDraft`/`saveCompanyDocumentDraft`/`publishCompanyDocument` implementan el bucle completo — abrir es idempotente y copia de la vigente, guardar edita en sitio sin versionar, publicar degrada y promueve en una transacción con lock. Verificado contra la base real con los diez pasos del ciclo: guardar dos veces dejó una sola fila, publicar promovió y degradó. **Advertencia de precisión**: mismo método de verificación que DOC-03 arriba — forma de la transacción contra la base, no las acciones invocadas de verdad; el plan 03-06 firma este criterio por la interfaz con un humano delante) |
...
| DOC-02 | Phase 3 | Pending (conversión + ruta listas desde el plan 03-03; falta cablearlo en el editor del plan 03-06) |
...
*Last updated: 2026-09-02 — DOC-03, DOC-05 y DOC-06 completados (plan 03-05)*
