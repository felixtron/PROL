...
| DOC-01 | Phase 3 | Pending — UI construida y aprobada por el usuario; sin evidencia de base (03-01 puso el esquema; 03-04 puso `updateManualDocumentBody` con su política de versión, verificada contra la base; **03-06** puso `DocumentBodyEditor` con vista previa y el mensaje "sin cambios"/"pasa a la versión N", revisado y aprobado por el usuario en el checkpoint de la tarea 3. **Advertencia de precisión**: tras la aprobación, `P-RFC-4.1-01` sigue en `template_version=1` con su `contentHtml` original — la base no muestra que el paso "cambiar un párrafo y guardar" se haya completado nunca. No se cierra hasta que exista una versión de plantilla mayor a 1 generada por la interfaz, ver `03-06-SUMMARY.md`) |
| DOC-03 | Phase 3 | Pending — contrato de servidor listo y verificado; UI construida y aprobada; sin evidencia de base (03-05: `issueCompanyDocument` congela `contentHtml` y `sourceTemplateVersion` al emitir, dentro de la misma transacción con lock que degrada la `VIGENTE` anterior. Verificado contra la base real: editar la plantilla después de emitir a dos empresas no movió ni un carácter de los dos snapshots, comparados antes/después. **Advertencia de precisión (03-05)**: esa verificación fue por script desechable que replica la forma de la transacción, no por invocar `issueCompanyDocument` de verdad. **Advertencia de precisión (03-06)**: el panel de empresas que cablea `issueCompanyDocument` a un botón fue construido y el checkpoint de la tarea 3 fue aprobado ("aprobado"), pero `company_documents` sigue en **0 filas** tras la aprobación — ninguna emisión real quedó registrada. No se cierra hasta que exista al menos una fila `VIGENTE` con `sourceTemplateVersion` generada por la interfaz, ver `03-06-SUMMARY.md`) |
...
| DOC-05 | Phase 3 | Pending — contrato de servidor listo y verificado; falta la interfaz (03-05: `getCompanyDocumentForClient`/`getCompanyDocumentForEdit` generan el historial —versión, fecha, autor, `notes` como descripción del cambio, estatus— en tiempo de render desde las filas de `CompanyDocument`, sin que nadie lo redacte a mano. Verificado contra la base real: cuatro filas de historial para Acme tras el ciclo completo, con las cinco columnas rellenas y al menos una descripción de cambio real. 03-06 construyó la tabla que lo muestra en `companies/[assignmentId]/page.tsx`; la vista del cliente sigue en 03-07) |
| DOC-06 | Phase 3 | Pending — contrato de servidor listo y verificado; UI construida y aprobada; sin evidencia de base (03-05: `startCompanyDocumentDraft`/`saveCompanyDocumentDraft`/`publishCompanyDocument` implementan el bucle completo — abrir es idempotente y copia de la vigente, guardar edita en sitio sin versionar, publicar degrada y promueve en una transacción con lock. Verificado contra la base real con los diez pasos del ciclo: guardar dos veces dejó una sola fila, publicar promovió y degradó. **Advertencia de precisión (03-05)**: mismo método de verificación que DOC-03 arriba — forma de la transacción, no las acciones invocadas de verdad. **Advertencia de precisión (03-06)**: el botón "Editar"/"Continuar borrador"/"Publicar" quedó cableado y el checkpoint de la tarea 3 —diseñado explícitamente para firmar este criterio con un humano delante— fue aprobado, pero `company_documents` sigue en **0 filas**: ni un borrador ni una publicación quedaron registrados. No se cierra hasta que la base muestre el ciclo guardar-dos-veces-publicar ejecutado por la interfaz, ver `03-06-SUMMARY.md`) |
...
| DOC-02 | Phase 3 | Pending — conversión probada, UI cableada y aprobada; sin evidencia de base (la conversión `.docx` → HTML saneado con tablas ya está probada contra un archivo real desde el plan 03-03, independiente de la UI. 03-06 cableó el importador en `DocumentBodyEditor` —FormData, vista previa automática, aviso de imágenes descartadas— y el checkpoint de la tarea 3 fue aprobado. **Advertencia de precisión**: el contenido de `P-RFC-4.1-01` en la base sigue siendo el original de la fixture, sin cambios — no hay evidencia de que una importación real se haya guardado por la interfaz. No se cierra hasta que un `.docx` real, importado por la interfaz, deje sus tablas en `contentHtml`, ver `03-06-SUMMARY.md`) |
...
*Last updated: 2026-09-02 — 03-06 construyó y el usuario aprobó la UI de DOC-01/02/03/06, pero `company_documents` sigue en 0 filas y `P-RFC-4.1-01` sigue en `template_version=1` tras esa aprobación: ninguno de los cuatro se cierra hasta que exista evidencia real de base de datos (ver `03-06-SUMMARY.md`). DOC-04/05/07 siguen pendientes de la vista del cliente en 03-07.*
